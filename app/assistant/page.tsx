...
export default function AssistantPage() {
  ...
  return (
    <AppShell>
      <div className="flex flex-col h-[calc(100vh-140px)] min-h-[550px] bg-white rounded-xl border border-slate-200/90 shadow-2xs overflow-hidden">
        {/* Chat Header */}
        <div className="p-4 sm:px-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/60">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg bg-[#002045] flex items-center justify-center text-white shadow-2xs">
          ...
          <button
            ...
            className="flex items-center gap-1 text-[11px] text-slate-500 hover:text-slate-800 px-2.5 py-1 rounded border border-slate-200 hover:bg-slate-100 transition-colors cursor-pointer"
            title="Reset Chat"
          >
        ...
        <div className="px-4 sm:px-6 py-2 bg-[#f8f9ff] border-b border-slate-100 flex items-center gap-2 overflow-x-auto">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {PRESET_PROMPTS.map((prompt, idx) => (
              <button
                ...
                className="px-2.5 py-1 text-xs font-medium rounded-full bg-white text-slate-700 hover:bg-[#002045] hover:text-white border border-slate-200 transition-colors cursor-pointer disabled:opacity-50 shrink-0"
              >
                {prompt}
              </button>
            ))}
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5">
          {messages.map((msg) => {
            ...
            return (
              ...
              >
                {!isUser && (
                  <div className="w-8 h-8 rounded-lg bg-[#002045] flex items-center justify-center text-white shrink-0 text-xs font-bold shadow-2xs">
                ...
                <div className="flex flex-col gap-1 max-w-xl">
                  <div
                    className={`p-4 rounded-xl text-xs leading-relaxed ${
                      isUser
                        ? "bg-[#002045] text-white rounded-br-none shadow-2xs"
                        : "bg-slate-50 text-slate-800 border border-slate-200/90 rounded-bl-none"
                    }`}
                  >
                    ...
                    {!isUser && msg.sources && msg.sources.length > 0 && (
                      <div className="mt-3 pt-2.5 border-t border-slate-200/80">
                        <button
                          ...
                          className="flex items-center justify-between w-full text-[11px] font-semibold text-slate-600 hover:text-slate-900 transition-colors"
                        >
                        ...
                        {isSourcesExpanded && (
                          <div className="mt-2 space-y-1.5 animate-in fade-in duration-200">
                            {msg.sources.map((src, i) => (
                              <div
                                key={i}
                                className="p-2 rounded bg-white border border-slate-200 text-[10px] text-slate-600 flex items-start justify-between gap-2"
                              >
                                ...
                                <span className="px-1.5 py-0.5 rounded bg-slate-100 text-slate-700 font-mono shrink-0">
          ...
          {loading && (
            <div className="flex gap-3 max-w-xl mr-auto">
              <div className="w-8 h-8 rounded-lg bg-[#002045] flex items-center justify-center text-white shrink-0">
                <Bot className="w-4 h-4 text-[#38BDF8]" />
              </div>
              <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/90 text-xs text-slate-600 flex items-center gap-2">
                <Loader2 className="w-3.5 h-3.5 animate-spin text-[#006a61]" />
          ...
          {error && (
            <div className="p-3 bg-red-50 border border-red-200 rounded-lg text-xs text-red-800 flex items-start gap-2 max-w-xl">
        ...
        <div className="p-4 sm:p-5 bg-white border-t border-slate-100">
          ...
          >
            <input
              ...
              disabled={loading}
              className="flex-1 px-4 py-2.5 text-xs bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#002045] focus:bg-white text-slate-900 disabled:opacity-50"
            />
            <button
              type="submit"
              disabled={loading || !inputQuery.trim()}
              className="px-4 py-2.5 bg-[#002045] hover:bg-[#1a365d] text-white rounded-lg text-xs font-semibold shadow-2xs transition-colors flex items-center gap-1.5 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <span>Send</span>
          ...
          <div className="mt-2 text-[10px] text-slate-400 text-center flex items-center justify-center gap-1">
            <ShieldCheck className="w-3 h-3 text-[#006a61]" />
